...
#pragma omp parallel for default(none) private(ii) shared(src, dst, recon_obj, e, num, ref_N, ref_S, ref_RO, ref_E1, ref_E2, RO, E1, E2, dstCHA, srcCHA, convKRO, convKE1, convKE2, kRO, kNE1, kNE2, fitItself) if(num>1)
    ...
    void GenericReconCartesianGrappaGadget::perform_unwrapping(IsmrmrdReconBit &recon_bit, ReconObjType &recon_obj,
        ...
        GDEBUG_STREAM("RO: " << RO << ", E1: " << E1 << ", E2: " << E2 << ", dstCHA: " << dstCHA
            << ", N: " << N << ", S: " << S << ", SLC: " << SLC);
        GDEBUG_STREAM("ref_RO: " << ref_RO << ", ref_E1: " << ref_E1 << ", ref_E2: " << ref_E2
            << ", srcCHA: " << srcCHA << ", ref_N: " << ref_N << ", ref_S: " << ref_S << ", ref_SLC: " << ref_SLC);
        ...
        hoNDArray<std::complex<float> > tmp_all(RO, E1, E2, srcCHA, N, S, SLC);
        ...
        this->compute_image_header(recon_bit, recon_obj.recon_res_, e);
        IsmrmrdImageArray ismrdIm;
        ismrdIm.headers_ = recon_obj.recon_res_.headers_;
        ismrdIm.meta_ = recon_obj.recon_res_.meta_;
#pragma omp parallel default(none) private(ii) shared(num, N, S, RO, E1, E2, srcCHA, convkRO, convkE1, convkE2, ref_N, ref_S, recon_obj, dstCHA, unmixingCoeff_CHA, e, tmp_all) if(num>1)
        {
#pragma omp for
            for (ii = 0; ii < num; ii++) {
                size_t slc = ii / (N * S);
                ...
                T *pTmp = &(tmp_all(0, 0, 0, 0, n, s, slc));
                hoNDArray<std::complex<float> > res(RO, E1, E2, 1, pRes);
                hoNDArray<std::complex<float> > tmp(RO, E1, E2, srcCHA, pTmp);
                ...
                hoNDArray<std::complex<float> > aliasedIm(RO, E1, E2,
                                                          ...
                                                          1, pIm);
                Gadgetron::apply_unmix_coeff_aliased_image_3D(aliasedIm, unmixing, tmp);

                Gadgetron::sum_over_dimension(tmp, res, 3);
                
            }
        }
        ismrdIm.data_ = tmp_all;
        this->send_out_image_array(ismrdIm, e, image_series.value() + 10 * ((int)e + 2), GADGETRON_IMAGE_REGULAR);
