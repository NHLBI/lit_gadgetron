...
def prepare_sequence_actions(args, config):
    ...
    def prepare_sequence_action(section):
        ...
        # If skipping Siemens conversion, replace 'siemens' with 'copy'
        # Check for skip conversion flag in config
        if (
            'additional_arguments' in config[section]
            and 'skip_converstion' == config[section]['additional_arguments']
            and action_key == 'siemens'
        ):
            action_key = 'copy'
            # Patch the config so copy uses the Siemens data_file as source
            config[section]['source'] = config[section]['data_file']
        return action_factories.get(action_key)(section)
