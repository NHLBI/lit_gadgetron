...
int main(int argc, char **argv)
{
    ...
    desc.add_options()("help,h", "Produce help message")                                                          //
        ...
        ("RO,RO", po::value<size_t>(&RO)->default_value(0.05), "RO") 
    ...
    boost::shared_ptr<hoNDArray<vector_td<float, 3>>> trajectory_in =read_nd_array<floatd3>((char *)trajectory_file.c_str());

    boost::shared_ptr<hoNDArray<float>> dcf_in =read_nd_array<float>((char *)dcw_file.c_str());

    boost::shared_ptr<hoNDArray<float>> recon_dims_read =read_nd_array<float>((char *)recondim_file.c_str());

    boost::shared_ptr<hoNDArray<float>> fov_file_read =read_nd_array<float>((char *)fov_file.c_str());

    boost::shared_ptr<hoNDArray<float>> erecon_dims_read =read_nd_array<float>((char *)erecondim_file.c_str());

    boost::shared_ptr<hoNDArray<float_complext>> csm_in =read_nd_array<float_complext>((char *)csm_file.c_str());

    boost::shared_ptr<hoNDArray<float>> def_in =read_nd_array<float>((char *)def_file.c_str());

    boost::shared_ptr<hoNDArray<float>> invdef_in =read_nd_array<float>((char *)invdef_file.c_str());

    hoNDArray<size_t> shots_per_time =hoNDArray<size_t>(*read_nd_array<float>((char *)shotspertime_file.c_str()));

    hoNDArray<float_complext> hocw =hoNDArray<float_complext>(*read_nd_array<float_complext>((char *)combination_weights_file.c_str()));

    hoNDArray<float> hosct =hoNDArray<float>(*read_nd_array<float>((char *)scaled_time_file.c_str()));

    hoNDArray<float> hofbins =hoNDArray<float>(*read_nd_array<float>((char *)fbins_file.c_str()));

    boost::shared_ptr<hoNDArray<float_complext>> data_in =read_nd_array<float_complext>((char *)data_file.c_str());
    ...
    cuNDArray<Gadgetron::float_complext> kspace_data;
    ...
    */
    ...
    auto channel_images = (cuNDArray<float_complext>(data_in->get_dimensions()));
    cudaMemcpy(channel_images.get_data_ptr(), data_in->get_data_ptr(), data_in->get_number_of_elements() * sizeof(float_complext), cudaMemcpyHostToDevice);
    ...
    GDEBUG_STREAM("Channels Image shape: X " << data_in.get()->get_size(0) << " Y " << data_in.get()->get_size(1) << " Z " << data_in.get()->get_size(2) << " CHA " << data_in.get()->get_size(3));
    

    auto [traj_csm, dcf_csm] = reconstruction.organize_traj_dcw_hoNDArray(trajectory_in.get(), dcf_in.get(), calculateDCF, calculateKPRECOND);
         
    //GDEBUG_STREAM("Channel images size: " << channel_images.get_size(0) << " " << channel_images->get_size(1) << " " << channel_images->get_size(2) << " " << channel_images->get_size(3));
    square_inplace(&dcf_csm);
    size_t total_shots = size_t(traj_csm.get_size(0)/recon_params.RO);
    GDEBUG_STREAM("Total shots: " << total_shots);
    std::vector<size_t> ddims={recon_params.RO,total_shots,recon_params.numberChannels};
    kspace_data.create(ddims);
    fill(&kspace_data,float_complext(0.0,0.0));
    
    channel_images *= *conj(csm.get());
    auto combined = sum(&channel_images, channel_images.get_number_of_dimensions() - 1);
    reconstruction.deconstruct(combined.get(),&kspace_data, &traj_csm, &dcf_csm,csm.get());

    GDEBUG_STREAM("Writing Image");
    nhlbi_toolbox::utils::write_gpu_nd_array<float_complext>(kspace_data, out_file + std::string("_data.complex"));
