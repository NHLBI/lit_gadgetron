...
public:
    SpiralAccumulateFast(const Core::Context &context, const Core::GadgetProperties &props) : ChannelGadget<Acquisition>(context, props), header{context.header}, trajParams{context.header}
    {
    }
    ...
    hoNDArray<std::complex<float>> cropData(hoNDArray<std::complex<float>> &data, size_t extrasamples)
    {
        using namespace Gadgetron::Indexing;

        hoNDArray<std::complex<float>> data_short(data.get_size(0) - extrasamples, data.get_size(1));
        size_t start_index = 20 ; // hardcoded fix this
        for (int ii = start_index; ii < data_short.get_size(0) + extrasamples - start_index; ii++)
        {
            data_short(ii - start_index, slice) = data(ii, slice);
        }
        return data_short;
    }
    ...
    void process(InputChannel<Acquisition> &in, OutputChannel &out) override
    {
        ...
        if (generateTraj)
        {
            ...
            {
                trajParams.read_girf_kernel(GIRF_folder);
                trajParams.set_girf_sampling_time(GIRF_samplingtime);
            }
            tw_gen = trajParams.calculate_trajectories_and_weight(acq_header);
            trajgen = std::get<0>(tw_gen);
            dcwgen = std::get<1>(tw_gen);
            //normalize_trajectory(&trajgen);
            ...
            traj_dcw = nhlbi_toolbox::utils::separate_traj_and_dcw_3D_gen(&traj, &densitycw,
                                                    acq_header.idx.kspace_encode_step_2, ((float)this->header.encoding.front().encodedSpace.matrixSize.z));
        }
        /* Take trajectories from waveform to trajectory */
        else
            traj_dcw = nhlbi_toolbox::utils::separate_traj_and_dcw_3D(&(*traj), acq_header.idx.kspace_encode_step_2, ((float)this->header.encoding.front().encodedSpace.matrixSize.z));

        if (generateTraj)
        {
            auto extraSamples = data.get_size(0) - hoNDArray<floatd3>(*std::get<0>(traj_dcw).get()).get_size(0);
            data = cropData(data, size_t(extraSamples));
        }
        ...
        acquisitions.find(acq_header.idx.phase)->second.push_back(hoNDArray<float_complext>(std::move(data)));
        ...
        for (auto [acq_header, data, traj] : in)
        {
            ...
            if (acq_count == (fixedINT * maxZencode))
            {
                ...
                if ((acq_count == (fixedINT * maxZencode) || maxAcq < (fixedINT * maxZencode)) && !csmSent)
                {
                    ...
                    auto combined_acquisitions = nhlbi_toolbox::utils::concat<float_complext>(acquisitions_concat);
                 ...
                 //   combined_acquisitions = permute(combined_acquisitions, {0, 2, 1, 3}); // RO INT CHA SLICE
                    cudaSetDevice(0);
                    // auto combined_density = nhlbi_toolbox::utils::concat<float>(density_weights);
                    ...
                    auto combined_traj = nhlbi_toolbox::utils::concat<floatd3>(trajectories_concat_all);
                    combined_traj.reshape(combined_traj.get_size(0),
                                          -1,
                                          1);

                    //  combined_density = nhlbi_toolbox::utils::estimateDCF(combined_traj, nhlbi_toolbox::utils::concat<float>(dcf_concat_all), true, oversampling_factor_, iterations_dcf, image_dims_, false);
                    combined_density = nhlbi_toolbox::utils::concat<float>(dcf_concat_all);
                    //auto combined_density = nhlbi_toolbox::utils::concat<float>(dcw);
        ...
        if (!csmSent)
        {
            ...
            if ((acq_header.isFlagSet(ISMRMRD::ISMRMRD_ACQ_LAST_IN_MEASUREMENT) || acq_count == (fixedINT * maxZencode) || maxAcq < (fixedINT * maxZencode)) && !csmSent)
            {
               ...
               // combined_acquisitions = permute(combined_acquisitions, {0, 2, 1, 3}); // RO INT CHA SLICE
                ...
                combined_density = nhlbi_toolbox::utils::concat<float>(dcf_concat_all);
                combined_density.reshape(combined_traj.get_size(0),
                                         -1,
                                         1);
