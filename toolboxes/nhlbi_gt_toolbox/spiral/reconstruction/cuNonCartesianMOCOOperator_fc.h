...
namespace Gadgetron
{
  ...
  public:
    ...
    void set_doMC_iter(bool flag) { doMC_iter_ = flag; }
    void set_iteration_count(size_t count) { iteration_count = count; }
    ...
    void def_to_vec(cuNDArray<REAL> *deformation);
    void def_to_back_vec(cuNDArray<REAL> *deformation);
    virtual void set_refPhase( float refPhase ){ refPhase_ = refPhase; }
    virtual float get_refPhase(){ return refPhase_; }
    virtual void set_eligibleGPUs( std::vector<int> GPUs ){ eligibleGPUs = GPUs; }
    virtual std::vector<int> get_eligibleGPUs(){ return eligibleGPUs; }
    
  protected:
    size_t counter =0;
    size_t iteration_count = 10;
    bool doMC_iter_ = false;
