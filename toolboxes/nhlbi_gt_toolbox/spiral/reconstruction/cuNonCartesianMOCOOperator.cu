...
void cuNonCartesianMOCOOperator<REAL, D>::mult_M(cuNDArray<complext<REAL>>* in, cuNDArray<complext<REAL>>* out,
    ...
    for (size_t it = 0; it < shots_per_time_.size(); it++) {
        ...
        auto slice_view_in = gr.deform_image(&input, backward_deformation_[it]);
...
void cuNonCartesianMOCOOperator<REAL, D>::mult_MH(cuNDArray<complext<REAL>>* in, cuNDArray<complext<REAL>>* out,
    ...
    for (size_t it = 0; it < shots_per_time_.size(); it++) {
        ...
        output += gr.deform_image(&slice_view_output, forward_deformation_[it]);
    }
...
// __global__ static void deform_imageKernel(REAL* output, const REAL* vector_field, cudaTextureObject_t texObj, int
// width,
...
//        //        cubicTex3D(texObj, (0.5f + ixo), (REAL)(0.5f + iyo), (REAL)(0.5f + izo)));
...
// }
// template <class REAL, unsigned int D>
// cuNDArray<complext<REAL>> cuNonCartesianMOCOOperator<REAL, D>::deform_image(cuNDArray<complext<REAL>>* image,
...
template <class REAL, unsigned int D>
void cuNonCartesianMOCOOperator<REAL, D>::set_shots_per_time(hoNDArray<size_t> shots_per_time) {
