...
namespace nhlbi_toolbox
{
    ...
    {
        std::vector<arma::uvec> density_compensation::extractSlices(hoNDArray<floatd3> sp_traj, std::vector<size_t> image_dims)
        {
            ...
            //GDEBUG_STREAM("z_encodes.size()" << z_encodes.size())
            //GDEBUG_STREAM("z_encodes.size()" << image_dims[2])

            if (z_encodes.size() <= image_dims[2])
            {
                // z_encodes.print();
                ...
                slice_indexes = std::vector<arma::uvec>(image_dims[2] * 3);
        ...
        cuNDArray<float> density_compensation::estimate_DCF(cuNDArray<vector_td<float, D>> &traj, std::vector<size_t> image_dims)
        {
            ...
            auto temp = *(Gadgetron::estimate_dcw<float, D>(&flat_traj,
                                                            from_std_vector<size_t, D>(image_dims),
                                                            oversampling_factor_,
                                                            size_t(iterations), kernel_width_, ConvolutionType::ATOMIC));
        ...
        hoNDArray<float> density_compensation::estimate_DCF_slice(hoNDArray<floatd3> &sp_traj, hoNDArray<float> &sp_dcw, std::vector<size_t> image_dims , bool do3D)
        {
            hoNDArray<float> hodcw(sp_dcw.get_size(0), sp_dcw.get_size(1));
            auto slice_index = extractSlices(sp_traj, image_dims);
            if ((slice_index.size() <= image_dims[2]) && !do3D)
            {
               //  GadgetronTimer timer("DCF1: ");
...
#pragma omp parallel for num_threads(eligibleGPUs.size())
            ...
            else
            {
                ...
                cudcw = estimate_DCF<3>(cutraj, cudcw, image_dims);
                hodcw = *cudcw.to_host();
                hodcw.reshape(sp_traj.get_size(0), -1);
            }
            return hodcw;
        }
        hoNDArray<float> density_compensation::estimate_DCF_slice(hoNDArray<floatd3> &sp_traj, std::vector<size_t> image_dims, bool do3D)
        {
...
#pragma omp parallel for num_threads(eligibleGPUs.size())
            ...
            {
                auto eligibleGPUs = nhlbi_toolbox::utils::FindCudaDevices(sp_traj.get_number_of_elements() * 4 * 10);
                cudaSetDevice(eligibleGPUs[0]);
                auto cutraj = cuNDArray<floatd3>(sp_traj);
                auto cudcw = estimate_DCF<3>(cutraj, image_dims);
                hodcw = *cudcw.to_host();
                hodcw.reshape(sp_traj.get_size(0), -1);
                cudaSetDevice(origDevice);
            }
