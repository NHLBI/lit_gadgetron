...
namespace nhlbi_toolbox
{
    namespace reconstruction
    {
        ...
        void noncartesian_reconstruction_fc::reconstruct_todevice(cuNDArray<float_complext> &data, cuNDArray<float_complext> &images, cuNDArray<floatd3> &trajectory_in, cuNDArray<float> &dcf_in, cuNDArray<float_complext> &csm, cuNDArray<float_complext> &cweights, cuNDArray<float> &scaled_time_in, arma::fvec fbins, int deviceNo, bool forward)
        {
            //  GadgetronTimer timer("reconstruct_todevice");
            ...
            if (forward)
            {
                ...
                auto ttraj = set_device(&trajectory_in, deviceNo);
                auto tdcf = set_device(&dcf_in, deviceNo);
                auto tcsm = set_device(&csm, deviceNo);
                auto tcweights = set_device(&cweights, deviceNo);
                auto tscaled_time_in = set_device(&scaled_time_in, deviceNo);
                cudaSetDevice(deviceNo);
            ...
            else
            {
                ...
                cuNDArray<float_complext> tdata(*(data.get_dimensions()), deviceNo);
                // GDEBUG_STREAM("Tdata made:" << deviceNo);
...
#pragma omp parallel for num_threads(eligibleGPUs.size())
        ...
        void noncartesian_reconstruction_fc::reconstruct(cuNDArray<float_complext> &data, cuNDArray<float_complext> &images, cuNDArray<floatd3> &trajectory_in, cuNDArray<float> &dcf_in, cuNDArray<float_complext> &csm, cuNDArray<float_complext> &cweights, cuNDArray<float> &scaled_time_in, arma::fvec fbins)
        {
            ...
            if (fbins.n_elem > 1)
            {
                //GDEBUG_STREAM("fbins.n_elem rcsm: " << fbins.n_elem);
                auto temp_image = images;
                for (int ii = 0; ii < fbins.n_elem; ii++)
                {
                    fill(&temp_image, float_complext(0, 0));
                    ...
                    noncartesian_reconstruction::reconstruct(&data, &temp_image, &trajectory_in, &dcf_in, &csm);
                ...
                //nhlbi_toolbox::utils::write_gpu_nd_array(images,"/opt/data/gt_data/images_ncr_fc.complex");
                demodulate_kspace(data, scaled_time_in, -1 * fbins[fbins.n_elem - 1]);
            }
            else
            {
                noncartesian_reconstruction::reconstruct(&data, &images, &trajectory_in, &dcf_in, &csm);
            }
        }
        ...
        void noncartesian_reconstruction_fc::reconstruct(cuNDArray<float_complext> &data, cuNDArray<float_complext> &images, cuNDArray<floatd3> &trajectory_in, cuNDArray<float> &dcf_in, cuNDArray<float_complext> &cweights, cuNDArray<float> &scaled_time_in, arma::fvec fbins)
        {
            ...
            {
                noncartesian_reconstruction::reconstruct(&data, &images, &trajectory_in, &dcf_in);
            }
        }
        ...
        void noncartesian_reconstruction_fc::deconstruct(cuNDArray<float_complext> &images, cuNDArray<float_complext> &data, cuNDArray<floatd3> &trajectory_in, cuNDArray<float> &dcf_in, cuNDArray<float_complext> &cweights, cuNDArray<float> &scaled_time_in, arma::fvec fbins)
        {
            ...
            for (int ii = 0; ii < fbins.n_elem; ii++)
            {
                fill(&temp_data, float_complext(0, 0));
        ...
        void noncartesian_reconstruction_fc::deconstruct(cuNDArray<float_complext> &images, cuNDArray<float_complext> &data, cuNDArray<floatd3> &trajectory_in, cuNDArray<float> &dcf_in, cuNDArray<float_complext> &csm, cuNDArray<float_complext> &cweights, cuNDArray<float> &scaled_time_in, arma::fvec fbins)
        {
            //        GadgetronTimer timer("Deconstruct (noncartRecon)");
            ...
            if (fbins.n_elem > 1)
            {
                //GDEBUG_STREAM("fbins.n_elem csm: " << fbins.n_elem);
                auto temp_data = data;
                for (int ii = 0; ii < fbins.n_elem; ii++)
                {
                    fill(&temp_data, float_complext(0, 0));
                    ...
                    noncartesian_reconstruction::deconstruct(&channel_images_temp, &temp_data, &trajectory_in, &dcf_in, &csm);
            ...
            {
                noncartesian_reconstruction::deconstruct(&images, &data, &trajectory_in, &dcf_in, &csm);
            }
        }
