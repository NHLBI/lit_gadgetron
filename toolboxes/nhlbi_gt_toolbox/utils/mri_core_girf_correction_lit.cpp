...
namespace nhlbi_toolbox
{
    namespace corrections
    {
                                                      ...
                                                      float girf_sampling_time, float TE) // Times are in us
        {
            ...
            for (int batch = 0; batch < nbatches; batch++)
            {
                ...
                if (maxTx > 100 || minTx < -100)
                    GERROR("gradients outside the required range");
                ...
                for (size_t k = 0; k < 3; k++)
                {
                    for (size_t i = nlines; i < nlines + filter.size() / 2; i++)
                    {
                        ...
                        {
                            GERROR("gradients outside the required range after filtration");
                        }
                    }
                ...
                for (size_t k = 0; k < 3; k++)
                {
                    for (size_t i = -std::floor(nlines_new/2); i < nlines_new/2; i++)
                    {
                        // Removed -ve sign needs more testing but was trying to match MATLAB implementation
                        rotated(i+std::floor(nlines_new/2), k) = rotated(i, k) * std::exp(2if * ADCShift * PI * frequency_resolution * float(i));
                    }
                }
        ...
        hoNDArray<std::complex<float>> load_girf_kernel(const std::string &girf_string)
        {
            ...
            bool success = nhlbi_toolbox::corrections::parse_girf_text(girf_string.begin(), girf_string.end(), temp_array);
            if (!success)
        ...
        hoNDArray<std::complex<float>> readGIRFKernel(std::string folder)
        {
            ...
            path fnamex = folder + "x.txt";
            path fnamey = folder + "y.txt";
            path fnamez = folder + "z.txt";
