...
def clear_cuda_cache(device_list=[]):
    """ Clear cuda memory
    """
    if len(device_list)==0:
        device_list = list(range(torch.cuda.device_count()))
    for devno in device_list:
        with torch.cuda.device(devno):
            gc.collect()
        ...
        with cp.cuda.Device(devno):
            ...
            mempool = cp.get_default_memory_pool()
            pinned_mempool = cp.get_default_pinned_memory_pool()
            print(f"GPU : {devno} Pinned Memory : {mempool.total_bytes()/(1024**2)} MB Used Pinned Memory : {mempool.used_bytes()/(1024**2)} MB")
