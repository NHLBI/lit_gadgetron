...
def estimateGatingSignal(nav_data,nav_tstamp,navangles,acq_tstamp,usegaussian=False):
...
def binning_div(selectedSig,timestamp,numRBins,evenBins,bidirectional):
    ...
    if(evenBins):
        ...
        delta = np.floor((high_idx-low_idx)/(numRBins))
        
        indices_sorted_min = np.floor(low_idx + delta*range(0,numRBins))
        indices_sorted_max = np.floor(low_idx + delta*range(1,numRBins+1))

    else:
        delta = (n95-n05)/numRBins
        min_limits = n05 + delta*range(0,numRBins)
        max_limits = n05 + delta*range(1,numRBins+1)
        
        indices_sorted_min = []
        indices_sorted_max = []
        
        for ii in range(0,numRBins):
...
def binning(selectedSig, timestamp, acceptancePercent, bidirectional, do_stable_binning, evenBins, numRBins):
    eprint("Binning")
    st = time.time()   
    ...
    if(do_stable_binning):
        ...
        indices, atimes = binning_div(selectedSig,timestamp,numRBins,evenBins,bidirectional)
...
def BinningGadget(connection):
    ...
    if "usegaussian" in params:
        if params["usegaussian"] == 'True':
            usegaussian = True
        else:
            usegaussian = False
    else:
            ...
            if not (acq.isFlagSet(mrd.ACQ_IS_RTFEEDBACK_DATA or mrd.ACQ_IS_HPFEEDBACK_DATA or mrd.ACQ_IS_NAVIGATION_DATA)):
                if useDC:
                    if(acq.idx.kspace_encode_step_2 == int(mrd_header.encoding[0].encodedSpace.matrixSize.z/2)):
                        ...
                        #nav_data.append(np.array(acq.data[:,6:15]))
                        nav_tstamp.append(acq.acquisition_time_stamp)
                        nav_indices.append(count-1)    
    ...
    st = time.time()   
    respiratory_waveform, samplingTime = estimateGatingSignal(nav_data_copy,nav_tstamp_copy,nav_angles_copy,acq_tstamp_copy,usegaussian) 
    cp.cuda.runtime.deviceSynchronize()
    ...
    #binning(selectedSig, timestamp, acceptancePercent, bidirectional, do_stable_binning, evenBins, numRBins):
    acceptedTimes = binning(respiratory_waveform,nav_tstamp,40,bidirectional, do_stable_binning, evenbins, numRBins )
    ...
    data = np.zeros((imageSize, numRBins,1,1))

    
    for ii in range(0,len(idx_to_send)):
        data[range(0,len(idx_to_send[ii])),ii%numRBins,int(ii/numRBins),0] = idx_to_send[ii].squeeze()
    
    # data = np.reshape(data,(imageSize, numRBins,1,1))
