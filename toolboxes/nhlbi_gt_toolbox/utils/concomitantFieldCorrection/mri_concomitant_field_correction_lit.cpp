/*
 * mri_concomitant_field_correction_lit.cpp
 *
...
namespace nhlbi_toolbox
{
    ...
    {
        mri_concomitant_field_correction_lit::mri_concomitant_field_correction_lit(const ISMRMRD::IsmrmrdHeader &head)
        {
            R_DCS_PCS = nhlbi_toolbox::utils::lookup_PCS_DCS(head.measurementInformation.get().patientPosition);

            auto str_model = (head.acquisitionSystemInformation).get().systemModel->c_str();

            if (strstr(str_model, "MAGNETOM eMeRge-XL") || strstr(str_model, "Investigational_Device_VE11S_S118") || strstr(str_model, "MAGNETOM Free.Max"))
                B0 = float(0.55f); // head.acquisitionSystemInformation.get().systemFieldStrength_T; -> Not giving the correct field value bug !
            else
                B0 = float(1.50f);

            GDEBUG_STREAM("B0: " << B0);
            fov(0) = head.encoding[0].encodedSpace.fieldOfView_mm.x / 1000.0f; // meters
            fov(1) = head.encoding[0].encodedSpace.fieldOfView_mm.y / 1000.0f; // meters
            fov(2) = head.encoding[0].encodedSpace.fieldOfView_mm.z / 1000.0f; // meters
           ...
           if (head.encoding[0].trajectoryDescription)
            {
                traj_desc = *head.encoding[0].trajectoryDescription;
                try
                {
                    auto userparam_long = to_map(traj_desc.userParameterLong);
                    auto userparam_double = to_map(traj_desc.userParameterDouble);
                    sampling_time = userparam_long.at("SamplingTime_ns") * 1e-9;
                    if (sampling_time == 0)
                        sampling_time = 2e-6; // need this because UTE sequence doesn't setup this variable

                    GDEBUG_STREAM("sampling_time: " << float(sampling_time));
                }
            ...
            {
                GDEBUG_STREAM("BSTAR: ")
                sampling_time = 2e-6; // need this for BSTAR
                GDEBUG_STREAM("sampling_time: " << float(sampling_time));
                //throw std::runtime_error("Trajectory description missing");
            }
        }

        void mri_concomitant_field_correction_lit::set_offset(ISMRMRD::AcquisitionHeader acq_header)
        {
            offset(0) = acq_header.position[0];
            offset(1) = acq_header.position[1];
            offset(2) = acq_header.position[2];
            offset = offset / 1000.0f; // convert to m
        }

        void mri_concomitant_field_correction_lit::set_rotationMatrix(ISMRMRD::AcquisitionHeader acq_header)
        {
        ...
        bool mri_concomitant_field_correction_lit::is_setup()
        {
            return isSetup;
        }
        void mri_concomitant_field_correction_lit::generate_mesh_grid(ISMRMRD::AcquisitionHeader acq_header)
        {
        ...
        void mri_concomitant_field_correction_lit::calculate_fieldCoordinates()
        {
        ...
        void mri_concomitant_field_correction_lit::calculate_scaledTime(hoNDArray<floatd3> gradients)
        {
        ...
        hoNDArray<float> mri_concomitant_field_correction_lit::calculate_gradCsum(hoNDArray<float> gradients)
        {
        ...
        hoNDArray<float> mri_concomitant_field_correction_lit::calculate_gradXYamp(hoNDArray<floatd3> gradients)
        {
        ...
        void mri_concomitant_field_correction_lit::calculate_freqMap()
        {
            // There may be a slight discrepancy with matlab please explore further later
            freqMap = float(1.0 / (float(4.0f) * B0)) * float(gamma) * (maxGA * maxGA) * field_cords;
        }
        void mri_concomitant_field_correction_lit::calculate_combinationWeights()
        {
            ...
            //nhlbi_toolbox::utils::write_cpu_nd_array(t, "/opt/data/gt_data/time.real");
            GDEBUG_STREAM("t size" << t.get_size(0));
            GDEBUG_STREAM("t size 1" << t.get_size(1));
            ...
            auto A = arma::cx_fmat(arma::cos(float(2.0) * float(3.1416) * time * this->demodulation_freqs.as_row()), arma::sin(float(2.0) * float(3.1416) * time * this->demodulation_freqs.as_row()));
            ...
            auto C_r = arma::cx_fmat(freqs.n_elem, demodulation_freqs.n_elem, arma::fill::zeros);
            C = arma::cx_fmat(freqMap.n_elem, demodulation_freqs.n_elem);

            // Code was breaking on server with 256 threads not sure why
            if (omp_get_max_threads() > 128)
                omp_set_num_threads(128);
            else
                omp_set_num_threads(64);
            {
                GadgetronTimer timer("Calc weights :");

#ifdef NDEBUG
                GDEBUG_STREAM("OMP num threads : " << omp_get_max_threads());

#pragma omp parallel for shared(C_r, R, Qh, freqs, time)
#endif
                for (auto ii = 0; ii < freqs.n_elem; ii++)
                {
                    auto b = arma::cx_fcolvec(arma::cos(float(2.0) * float(3.1416) * float(freqs(ii)) * time), arma::sin(float(2.0) * float(3.1416) * float(freqs(ii)) * time));
                    // GDEBUG_STREAM("b:" << b);
                  //  GDEBUG_STREAM("freqs(ii) :" << freqs(ii));
                    // GDEBUG_STREAM("b:" << b);
                    auto x = arma::cx_fvec(arma::pinv(R) * Qh * b);
                  //  GDEBUG_STREAM("x:" << x);
                    C_r.row(ii) = x.as_row();
                }
            }
            ...
            {
                GadgetronTimer timer("putting data in C :");
...
#pragma omp parallel for
        ...
        int mri_concomitant_field_correction_lit::find_index(arma::vec in, float val)
        {
        ...
        void mri_concomitant_field_correction_lit::set_combinationWeights()
        {
...
#pragma omp parallel for
        ...
        void mri_concomitant_field_correction_lit::setup(hoNDArray<floatd3> gradients, ISMRMRD::AcquisitionHeader acq_header)
        {
            ...
            demodulation_freqs = arma::linspace<arma::fvec>(freqMap.min() - 0.2f * freqMap.min(), freqMap.max() * 1.2f, numfreqbins);
