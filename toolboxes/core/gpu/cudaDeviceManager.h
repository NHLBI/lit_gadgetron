...
namespace Gadgetron{
  ...
  template<class T>
  struct GT_cudaDataType {
  };

  template<>
  struct GT_cudaDataType<float>{
    static constexpr cudaDataType_t value = CUDA_R_32F;
  };

  template<>
  struct GT_cudaDataType<double>{
    static constexpr cudaDataType_t value = CUDA_R_64F;
  };

  template<>
  struct GT_cudaDataType<complext<float>>{
    static constexpr cudaDataType_t value = CUDA_C_32F;
  };

  template<>
  struct GT_cudaDataType<complext<double>>{
    static constexpr cudaDataType_t value = CUDA_C_64F;
  };


  template<class T>
  constexpr cudaDataType_t cuda_datatype(){return GT_cudaDataType<T>::value;}
