...
#include <gadgetron/NHLBICompression.h>
...
enum GadgetronMessageID {
    ...
    GADGET_MESSAGE_ISMRMRD_FEEDBACK                       = 1028,
...
class GadgetronClientFeedbackMessageReader 
    : public GadgetronClientMessageReader
{

public:
    GadgetronClientFeedbackMessageReader()

    {

    }

    virtual ~GadgetronClientFeedbackMessageReader() {}
    ...
    virtual void read(tcp::socket* socket) 
    {
        ...
        boost::asio::read(*socket, boost::asio::buffer(&size_str, sizeof(uint32_t)));
                std::cout << "size_str: "            << size_str << std::endl;

        char name_message[size_str];
        boost::asio::read(*socket, boost::asio::buffer(&name_message, size_str));
        boost::asio::read(*socket, boost::asio::buffer(&size_data, sizeof(uint32_t)));
        std::cout << "Message: "<< name_message << std::endl;
        std::cout << "size_data: "            << size_data << std::endl;
        std::string mymsg((name_message));
        if (mymsg =="MyFeedback"){
            Gadgetron::FeedbackData fdata;
            boost::asio::read(*socket, boost::asio::buffer(&fdata, size_data));
            std::cout << "Feedback mybool: "    << fdata.myBool << std::endl;
            std::cout << "Feedback myint: "     << fdata.myints[0] << std::endl;
            std::cout << "Feedback myint2: "    << fdata.myints[1] << std::endl;
            std::cout << "Feedback myfloat: "   << fdata.myFloat << std::endl;
        }

        if (mymsg=="MyCalibratedVENC"){
            Gadgetron::CalibratedVENCData fdata;
            boost::asio::read(*socket, boost::asio::buffer(&fdata, size_data));
            std::cout << "Feedback myint: "    << fdata.myint << std::endl;
        }

        if (mymsg=="MyQCPSF"){
            Gadgetron::QCPSFData fdata;
            boost::asio::read(*socket, boost::asio::buffer(&fdata, size_data));
            std::cout << "Feedback myfloat: "   << fdata.myFloat << std::endl;
        }
...
int main(int argc, char **argv)
{
    ...
    con.register_reader(GADGET_MESSAGE_ISMRMRD_FEEDBACK, std::shared_ptr<GadgetronClientMessageReader>(new GadgetronClientFeedbackMessageReader()));
