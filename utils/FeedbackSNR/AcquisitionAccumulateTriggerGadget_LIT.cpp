#include "AcquisitionAccumulateTriggerGadget_LIT.h"
#include "log.h"
...
#include <chrono>
namespace Gadgetron {
    using TriggerDimension = AcquisitionAccumulateTriggerGadget_LIT::TriggerDimension;
    namespace {
        ...
        Trigger get_trigger(const AcquisitionAccumulateTriggerGadget_LIT& gadget) {
    ...
    void AcquisitionAccumulateTriggerGadget_LIT::send_data(Core::OutputChannel& out, std::map<unsigned short, AcquisitionBucket>& buckets,
    ...
    void AcquisitionAccumulateTriggerGadget_LIT ::process(
        ...
        for (auto message : in) {
            ...
            if (((head.isFlagSet(ISMRMRD::ISMRMRD_ACQ_LAST_IN_SLICE) & head.isFlagSet(ISMRMRD::ISMRMRD_ACQ_LAST_IN_REPETITION)) & head.isFlagSet(ISMRMRD::ISMRMRD_ACQ_LAST_IN_MEASUREMENT)) ){
                GDEBUG_STREAM("THIS IS THE LAST MEASUREMENT  \n" << idx);
                if (!trigger_one){
                    send_data(out, buckets, waveforms);
                    GDEBUG_STREAM("WHAT  \n" << idx);
                    trigger_one=true;
        ...
        GDEBUG_STREAM("THIS IS THE END OF THE ACQUISITION \n" << idx);
        if ( !trigger_one){
        send_data(out,buckets,waveforms);
        }
    ...
    GADGETRON_GADGET_EXPORT(AcquisitionAccumulateTriggerGadget_LIT);
