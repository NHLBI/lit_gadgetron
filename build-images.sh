...
image_tag="latest"
base_name="gadgetronnhlbi/ubuntu_2204_cuda128_lit"
...
for t in "${types[@]}"; do
  for f in "${flavors[@]}"; do
    image_name="${base_name}:${image_tag}_${t}_${f}"
    build_stage="gadgetron_${t}_${f}"
    docker build --build-arg BUILDKIT_INLINE_CACHE=1 --target "$build_stage" -t "$image_name" -f "$(dirname "$0")/Dockerfile" . 
    #"$(dirname "$0")" 
    if [[ -n "${push:-}" ]]; then
